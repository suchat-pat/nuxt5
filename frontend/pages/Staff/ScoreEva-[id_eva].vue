                        ...
                        <v-col cols="12">
                            ...
                            <v-table class="table">
                                <tr>
                                    ...
                                    <th class="pa-2 border bg-grey text-black" style="width: 10%;">ไฟล์</th>
                                ...
                                <tr v-for="indicate in topic.indicates" :key="indicate.id_indicate">
                                    ...
                                    <td class="border text-center pa-2"><v-btn v-if="indicate.file_eva" @click="viweFile(indicate.file_eva)" color="blue">ดูไฟล์</v-btn><span v-else>-</span></td>
                                    <td class="border text-center pa-2">{{ indicate.score_member * indicate.point_indicate }}</td>
                                </tr>
                            </v-table>
                        </v-col>
                    </v-row>
...
import {staff,eva} from '../../API/base'
...
const fetchTopic = async () =>{
    try{
        const token = localStorage.getItem('token')
        const res = await axios.get(`${staff}/score_member/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})
